          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="sm:flex sm:items-start">
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  ...
                  <div className="mt-4 space-y-4">
                      ...
                      <div className="max-h-40 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-md">
                        {filteredProducts.map((product) => {
                          ...
                          return (
                            <div
                              key={product.id}
                              className={`p-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 flex justify-between ${maxStock <= 0 ? 'opacity-50' : ''}`}
                              onClick={() => maxStock > 0 && handleProductSelect(product)}
                            >
                              <span className="text-gray-900 dark:text-white">{product.nombre}</span>
                              <span className="text-sm text-gray-500 dark:text-gray-400">Disp: {maxStock}</span>
                            </div>
                          );
                        })}
                      </div>
                    ...
                    <div className="mt-4">
                      <h4 className="text-sm font-medium text-gray-900 dark:text-white mb-2">Productos a transferir:</h4>
                      {selectedProducts.length === 0 ? (
                        <p className="text-sm text-gray-500">No hay productos seleccionados.</p>
                      ) : (
                        <div className="space-y-2">
                          {selectedProducts.map((p) => (
                            <div key={p.producto_id} className="flex items-center justify-between bg-gray-50 dark:bg-gray-900 p-2 rounded">
                              <span className="text-sm text-gray-900 dark:text-white flex-1">{p.nombre} (Máx: {p.maxStock})</span>
                              <div className="flex items-center space-x-2">
                                <input
                                  ...
                                  className="w-20 p-1 text-sm border-gray-300 rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                />
                                <button
                                  type="button"
            ...
            <div className="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
              ...
              <button
                type="button"
                onClick={onClose}
                className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-900"
              >
                Cancelar
              </button>
            </div>
