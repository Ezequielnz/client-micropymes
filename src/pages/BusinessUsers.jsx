...
const mapSettingsToFormState = (settings) => ({
  inventario_modo: settings?.inventario_modo || 'por_sucursal',
  servicios_modo: settings?.servicios_modo || 'por_sucursal',
  catalogo_producto_modo: settings?.catalogo_producto_modo || 'por_sucursal',
  permite_transferencias: settings?.permite_transferencias ?? true,
  transferencia_auto_confirma: settings?.transferencia_auto_confirma ?? false,
  default_branch_id: settings?.default_branch_id ? String(settings.default_branch_id) : '',
});
...
const BranchFormModal = ({
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // La capa superior muestra el error.
    }
  };
...
const BranchSettingsPanel = ({ branches, settings, saving, error, onSave }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (formState.inventario_modo !== settings.inventario_modo) {
      changes.inventario_modo = formState.inventario_modo;
    }
    if (formState.servicios_modo !== settings.servicios_modo) {
      changes.servicios_modo = formState.servicios_modo;
    }
    if (formState.catalogo_producto_modo !== settings.catalogo_producto_modo) {
      changes.catalogo_producto_modo = formState.catalogo_producto_modo;
    }
    if (formState.permite_transferencias !== settings.permite_transferencias) {
      changes.permite_transferencias = formState.permite_transferencias;
    }
    if (formState.transferencia_auto_confirma !== settings.transferencia_auto_confirma) {
      changes.transferencia_auto_confirma = formState.transferencia_auto_confirma;
    }
    ...
    if (normalizedDefault !== currentDefault) {
      changes.default_branch_id = normalizedDefault;
    }
    ...
    <form onSubmit={handleSubmit} className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Controla si cada sucursal puede tener precios o visibilidad propios.
          </p>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
